...
<script type="text/javascript">
	//1.先删除
	UE.delEditor('privilege_container');
	//2.再加载
	var ue = UE.getEditor('privilege_container', {
...
function addprivilege(){
	...
	//获取添加角色名称
	var privilegeName = ue.getContentTxt();
	//两次url编码
	...
	//解码decodeURI(test1);
	$.post("/Forum/servlet/AddPrivilegeServlet?encode_content="+encode_content+"&purl="+purl, $("#add_privilege_form").serialize(),// 这是form的
			// id
			function(msg) {
				...
				if(data.message=="success"){
					//存入成功之后
		 			$("#tips").text("成功添加"+privilegeName+"权限");
	...
	<form id="add_privilege_form" method="post" onsubmit="return addprivilege()">
		<script id="privilege_container" name="content" type="text/plain">
		请输入权限名称
    </script>
  	<!-- 资源路径 -->
  	</br>
  	<div style="width:400px;height:36px;">
  		<input type="text" id="purl" value="请输入资源路径" onblur="if(this.value==''){this.value='请输入资源路径'}" onfocus="this.value=''" style="color: #444;width:400px;height:36px;"/>
  	</div>
