...
public class ImageFont extends HttpServlet
{
			...
			throws ServletException, IOException
	{
		...
		sb.setLength(0);//让stringbuffer清掉上次的缓存
	...
	private void drawRandomLine(Graphics g)
	{
		...
		{
			//起点坐标
			int x1 =new Random().nextInt(WIDTH);//范围在宽的范围之内
			int y1 =new Random().nextInt(HEIGHT);//范围在高的范围之内
	...
	private void drawRandomNum(Graphics2D g)
	{
		...
		for(int i = 0;i<4;i++)//随机产生4个汉字
		{
			int degree = new Random().nextInt()%30;//-30-30
			String ch=base.charAt(new Random().nextInt(base.length()))+"";//随机取一个汉字
			
			sb = sb.append(ch);
			g.rotate(degree*Math.PI/180, x, 20);//设置旋转的弧度    x代表基于这个点的横坐标，y则是宗坐标
			g.drawString(ch, x, 20);
			g.rotate(-degree*Math.PI/180, x, 20);//清除旋转的，为下次旋转做准备
			x += 30;
		}
